import csv
import sys
from collections import defaultdict, deque

import numpy as np

# 定数
C = 299792458          # 光速 [m/s]
DELTA_F = 1_000_000   # channel間隔 [Hz]


def load_raw_tone_csv(path, counter=None, antenna_path=0):
    """
    raw_tone.csvから[(channel, local_i, local_q, peer_i, peer_q), ...]を作る.
    basic版なのでquality_indicatorでは除外せず,local/peerが揃ったtoneを全部使う.
    """
    by_channel = defaultdict(lambda: {"local": deque(), "peer": deque()})
    headers = None
    total_rows = 0
    skipped_rows = 0
    skipped_incomplete = 0

    with open(path, newline="", encoding="utf-8-sig", errors="ignore") as f:
        for parts in csv.reader(f):
            if not parts:
                continue

            if parts[0] == "ranging_counter":
                headers = parts
                continue

            try:
                if headers and len(parts) >= len(headers):
                    row = {name: parts[index] for index, name in enumerate(headers)}
                elif len(parts) >= 9:
                    row = {
                        "ranging_counter": parts[0],
                        "channel": parts[2],
                        "side": parts[3],
                        "antenna_path": parts[4],
                        "i": parts[7],
                        "q": parts[8],
                    }
                elif len(parts) >= 8:
                    row = {
                        "ranging_counter": parts[0],
                        "channel": parts[2],
                        "side": parts[3],
                        "antenna_path": parts[4],
                        "i": parts[6],
                        "q": parts[7],
                    }
                else:
                    skipped_rows += 1
                    continue

                row_counter = int(row["ranging_counter"])
                row_channel = int(row["channel"])
                row_side = row["side"].strip()
                row_antenna_path = int(row.get("antenna_path", 0))
                row_i = int(row["i"])
                row_q = int(row["q"])
            except (KeyError, ValueError):
                skipped_rows += 1
                continue

            if counter is not None and row_counter != counter:
                continue

            if row_antenna_path != antenna_path:
                continue

            if row_side not in ("local", "peer"):
                skipped_rows += 1
                continue

            total_rows += 1
            by_channel[row_channel][row_side].append((row_i, row_q))

    tones = []

    for channel in sorted(by_channel):
        local_rows = by_channel[channel]["local"]
        peer_rows = by_channel[channel]["peer"]
        pair_count = min(len(local_rows), len(peer_rows))

        if pair_count == 0:
            skipped_incomplete += len(local_rows) + len(peer_rows)
            continue

        skipped_incomplete += abs(len(local_rows) - len(peer_rows))

        for _ in range(pair_count):
            il, ql = local_rows.popleft()
            ir, qr = peer_rows.popleft()
            tones.append((channel, il, ql, ir, qr))

    return tones, {
        "total_rows": total_rows,
        "skipped_rows": skipped_rows,
        "skipped_incomplete": skipped_incomplete,
    }

def calc_ifft_distance(tones, nfft=1024):
    """
    tones:
        [(channel, local_i, local_q, peer_i, peer_q), ...]
    """

    # 同じchannelが複数ある場合はSDKと同じように平均する
    avg_tones = {}
    counts = {}

    for ch, il, ql, ir, qr in tones:
        n = ch - 2

        if n < 0 or n >= 75:
            print(f"channel {ch} は範囲外なので無視します")
            continue

        counts[n] = counts.get(n, 0) + 1

        if counts[n] == 1:
            avg_tones[n] = [il, ql, ir, qr]
        else:
            count = counts[n]
            old = avg_tones[n]
            new = [il, ql, ir, qr]
            avg_tones[n] = [
                new_value / count + (1.0 - 1.0 / count) * old_value
                for old_value, new_value in zip(old, new)
            ]

    # IFFT入力配列
    X = np.zeros(nfft, dtype=complex)

    for n, (il, ql, ir, qr) in avg_tones.items():
        # localとpeerを複素数にする
        local = complex(il, ql)
        peer = complex(ir, qr)

        # SDKと同じ考え方：local × peer
        X[n] = local * peer

    # IFFT
    y = np.fft.ifft(X)

    # magnitude
    mag = np.abs(y)

    # 最大ピークindex
    k_peak = int(np.argmax(mag))

    # 3点補間
    early = mag[k_peak - 1]
    prompt = mag[k_peak]
    late = mag[k_peak + 1]

    if prompt >= early and prompt >= late:
        t_hat = (late - early) / (4 * prompt - 2 * (early + late))
    else:
        t_hat = 0.0

    # indexを距離に変換
    distance = (k_peak + t_hat) * C / (2 * nfft * DELTA_F)

    return distance, k_peak, t_hat, mag


def main():
    print("IFFT距離推定を行います")
    print("raw_tone.csvを読むか,手入力できます")
    print()

    if len(sys.argv) >= 2:
        csv_path = sys.argv[1]
        counter = int(sys.argv[2]) if len(sys.argv) >= 3 else None
        nfft = int(sys.argv[3]) if len(sys.argv) >= 4 else 1024
        tones, meta = load_raw_tone_csv(csv_path, counter)
    else:
        csv_path = input("raw_tone.csvのパスを入力してください．手入力なら空Enter：").strip()

        nfft_input = input("NFFTを入力してください．何も入力しない場合は1024：")

        if nfft_input.strip() == "":
            nfft = 1024
        else:
            nfft = int(nfft_input)

        if csv_path:
            counter_input = input("ranging_counterを入力してください．全件なら空Enter：")

            if counter_input.strip() == "":
                counter = None
            else:
                counter = int(counter_input)

            tones, meta = load_raw_tone_csv(csv_path, counter)
        else:
            print("入力形式：channel,local_i,local_q,peer_i,peer_q")
            print("例：2,152,59,153,-75")
            print("入力終了：何も入力せずEnter")
            print()

            tones = []
            meta = None

            while True:
                line = input("tone入力：")

                if line.strip() == "":
                    break

                try:
                    ch, il, ql, ir, qr = map(int, line.split(","))
                    tones.append((ch, il, ql, ir, qr))
                except ValueError:
                    print("入力形式が違います．例：2,152,59,153,-75")

    if len(tones) == 0:
        print("toneが入力されていません")
        return

    distance, k_peak, t_hat, mag = calc_ifft_distance(tones, nfft)

    print()
    print("===== 計算結果 =====")
    print(f"使用tone数：{len(tones)}")
    print(f"NFFT：{nfft}")
    print(f"k_peak：{k_peak}")
    print(f"t_hat：{t_hat:.6f}")
    print(f"IFFT距離：{distance:.6f} m")

    if meta is not None:
        print()
        print("===== CSV読み込み情報 =====")
        print(f"読み込み行数：{meta['total_rows']}")
        print(f"スキップ行数：{meta['skipped_rows']}")
        print(f"local/peer不足で除外した行数：{meta['skipped_incomplete']}")

    print()
    print("===== ピーク上位5個 =====")
    top_indices = np.argsort(mag)[-5:][::-1]

    for idx in top_indices:
        d = idx * C / (2 * nfft * DELTA_F)
        print(f"index={idx}, magnitude={mag[idx]:.6f}, 距離={d:.6f} m")


if __name__ == "__main__":
    main()
